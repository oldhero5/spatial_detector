...
class PinholeCamera:
    ...
    def pixel_to_3d(self, x, y, depth, normalized_depth=True, depth_scale=5.0):
        ...
        try:
            ...
            print(f"Error converting input types in pixel_to_3d: {e}, x={x}, y={y}, depth={depth}")
            return None
        ...
        try:
            ...
            if len(coordinates) != 3 or not all(isinstance(c, float) for c in coordinates):
                print(f"Warning: Created invalid coordinates: {coordinates}, setting to zeros")
                return (0.0, 0.0, 0.0)
    ...
    def load_calibration(self, calibration_file):
        ...
        try:
            ...
            if 'fx' in calibration and isinstance(calibration['fx'], (int, float)) and calibration['fx'] > 0:
                self.fx = float(calibration['fx'])
            else:
                print(f"Invalid fx value in calibration: {calibration.get('fx')}")
                
            if 'fy' in calibration and isinstance(calibration['fy'], (int, float)) and calibration['fy'] > 0:
            ...
            if 'depth_scale' in calibration and isinstance(calibration['depth_scale'], (int, float)) and calibration['depth_scale'] > 0:
