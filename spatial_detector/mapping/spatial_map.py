...
class SpatialMap:
    ...
    def get_topdown_view(self, width: int = 400, height: int = 400) -> np.ndarray:
        ...
        for obj_id, data in self.last_positions.items():
            ...
            pixel_y = int(
                camera_y - obj_z * scale_z
            )  # Invert Z because pixel y increases downward
            ...
            if 0 <= pixel_x < width and 0 <= pixel_y < height:
                ...
                if (
                    obj_id in self.object_history
                    and len(self.object_history[obj_id]) > 1
                ):
                    ...
                    for i in range(1, len(history)):
                        ...
                        if (
                            0 <= prev_px < width
                            and 0 <= prev_py < height
                            and 0 <= curr_px < width
                            and 0 <= curr_py < height
                        ):
                            cv2.line(
                                map_image,
                                (prev_px, prev_py),
    ...
    def _draw_compass(self, image: np.ndarray, width: int, height: int):
        ...
        cv2.line(
            image,
            (compass_x, compass_y),
            (compass_x, compass_y - compass_radius),
            (0, 0, 0),
            2,
        )
        cv2.putText(
            image,
            "N",
            (compass_x - 5, compass_y - compass_radius - 5),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.4,
            (0, 0, 0.0),
            1,
        ...
        cv2.line(
            image,
            (compass_x, compass_y),
            (compass_x + compass_radius, compass_y),
            (0, 0, 0),
            1,
        )
        cv2.putText(
            image,
            "E",
            (compass_x + compass_radius + 5, compass_y + 5),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.4,
            (0, 0, 0),
            1,
        ...
        cv2.putText(
            image,
            "S",
            (compass_x - 5, compass_y + compass_radius + 15),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.4,
            (0, 0, 0),
            1,
        ...
        cv2.putText(
            image,
            "W",
            (compass_x - compass_radius - 15, compass_y + 5),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.4,
            (0, 0, 0),
            1,
