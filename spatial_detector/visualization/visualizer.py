    ...
    def add_depth_visualization(self, frame, depth_normalized):
        ...
        try:
            ...
            if not isinstance(depth_normalized, np.ndarray):
                print(f"Error: depth_normalized is not a numpy array: {type(depth_normalized)}")
                return frame
            ...
            if depth_for_viz.max() > 0:
                depth_for_viz = (depth_for_viz - depth_for_viz.min()) / (depth_for_viz.max() - depth_for_viz.min())
                
            # Apply colormap to depth
            depth_colormap = cv2.applyColorMap(
                (depth_for_viz * 255).astype(np.uint8),
                cv2.COLORMAP_JET
            )
